            ...
            <script>
            	$(function() {
					...
					function getAttachStr(attach) {
						var str = "";
						str += '<li class="list-group-item" data-uuid="' + attach.uuid + '" data-path="' + attach.path + '" data-image="' + attach.image + '" data-origin="' + attach.origin + '"><a href="/download?' + $.param(attach) + '">' + attach.origin + '</a><button type="button" class="close"><span>&times;</span></button></li>'
						return str;
					}
					function getThumbStr(attach) {
						var str = "";
						var o = {...attach}; // clone문법
						o.uuid = 's_' + o.uuid;
